...
const C = {
  bg: "#080d1a", card: "#0f1829", border: "#1a2840",
  accent: "#00c896", accentDim: "#00c89615",
  gold: "#f0b429", goldDim: "#f0b42915",
  text: "#e2e8f4", muted: "#4a5f80",
  danger: "#f43f5e", green: "#22c55e",
};
...
function generatePDF(list, title) {
  ...
  const html = `<!DOCTYPE html><html><head><meta charset="utf-8"/><title>${list.name}</title><style>*{box-sizing:border-box;margin:0;padding:0}body{font-family:'Segoe UI',sans-serif;background:#fff;color:#1a1a2e;padding:40px;font-size:13px}.hd{display:flex;justify-content:space-between;border-bottom:3px solid #00c896;padding-bottom:20px;margin-bottom:28px}.br{font-size:22px;font-weight:800;color:#00c896}.rm{text-align:right;color:#666;font-size:12px}.rt{font-size:26px;font-weight:800;color:#0f172a;margin-bottom:20px}.sb{display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:32px}.sx{background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;padding:14px}.sl{font-size:10px;text-transform:uppercase;letter-spacing:1px;color:#94a3b8;margin-bottom:5px}.sv{font-size:18px;font-weight:700;color:#00c896;font-family:monospace}.prop{border:1px solid #e2e8f0;border-radius:12px;padding:20px;margin-bottom:20px;page-break-inside:avoid}.ph{display:flex;justify-content:space-between;margin-bottom:16px}.pn{font-size:10px;text-transform:uppercase;letter-spacing:1.5px;color:#f0b429;font-weight:700}.pa{font-size:16px;font-weight:700;color:#0f172a}.ps{font-size:12px;color:#64748b}.pp{font-size:20px;font-weight:800;font-family:monospace}.mg{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:14px}.mc{background:#f8fafc;border-radius:8px;padding:10px}.ml{font-size:9px;text-transform:uppercase;letter-spacing:1px;color:#94a3b8;margin-bottom:4px}.mv{font-size:16px;font-weight:700;font-family:monospace}.ms{font-size:10px;color:#94a3b8}.ac{color:#00c896}.go{color:#f0b429}.gr{color:#16a34a}.rd{color:#dc2626}.ab{background:#f0fdf9;border-left:3px solid #00c896;padding:10px 14px;border-radius:4px;font-size:12px;color:#334155;line-height:1.6}.ft{margin-top:40px;border-top:1px solid #e2e8f0;padding-top:16px;text-align:center;font-size:11px;color:#94a3b8}</style></head><body><div class="hd"><div><div class="br">PropAnalyzer</div></div><div class="rm"><strong>${list.name}</strong><br/>${dateStr}</div></div><div class="rt">${title||list.name}</div><div class="sb"><div class="sx"><div class="sl">Properties</div><div class="sv">${list.properties.length}</div></div><div class="sx"><div class="sl">Portfolio Value</div><div class="sv">${fmt$(totalValue)}</div></div><div class="sx"><div class="sl">Avg ST Monthly</div><div class="sv">${fmt$(Math.round(avgST))}</div></div><div class="sx"><div class="sl">Avg LT Monthly</div><div class="sv">${fmt$(Math.round(avgLT))}</div></div></div>${rows}<div class="ft">Generated by PropAnalyzer · For informational purposes only</div></body></html>`;
...
function Btn({ children, onClick, variant = "primary", disabled, small, full }) {
  ...
  return (
    <button onClick={onClick} disabled={disabled} style={{ ...s[variant], borderRadius: 8, padding: small ? "6px 12px" : "10px 20px", fontSize: small ? 12 : 13, fontWeight: 600, cursor: disabled ? "not-allowed" : "pointer", opacity: disabled ? 0.5 : 1, fontFamily: "inherit", whiteSpace: "nowrap", width: full ? "100%" : "auto" }}>
      {children}
    </button>
  );
}
    ...
    <div style={{ minHeight: "100vh", background: C.bg, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div style={{ width: "100%", maxWidth: 400 }}>
        <div style={{ textAlign: "center", marginBottom: 36 }}>
          <div style={{ fontSize: 22, fontWeight: 800, color: C.text, marginBottom: 6 }}>⬡ PropAnalyzer</div>
          ...
          <div style={{ display: "flex", marginBottom: 24, background: C.bg, borderRadius: 8, padding: 3 }}>
            {["login", "signup"].map(m => (
              <button key={m} onClick={() => { setMode(m); setError(""); setSuccess(""); }}
                style={{ flex: 1, background: mode === m ? C.card : "transparent", border: "none", color: mode === m ? C.text : C.muted, borderRadius: 6, padding: "7px", fontSize: 13, fontWeight: mode === m ? 600 : 400, cursor: "pointer" }}>
                {m === "login" ? "Log In" : "Sign Up"}
              </button>
            ))}
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            <Field label="Email" name="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="you@example.com" type="email" />
...
export default function App() {
  ...
  const analyzeProperty = async () => {
    ...
    try {
      ...
      if (!apiData.content) throw new Error("No content in response: " + JSON.stringify(apiData).slice(0, 300));
  ...
  return (
    <div style={{ display: "flex", height: "100vh", background: C.bg, color: C.text, fontFamily: "'Segoe UI', sans-serif" }}>
      ...
      {/* Sidebar */}
      <div style={{ width: 240, minWidth: 240, background: C.card, borderRight: `1px solid ${C.border}`, display: "flex", flexDirection: "column", height: "100vh" }}>
        <div style={{ padding: "18px 16px 14px", borderBottom: `1px solid ${C.border}` }}>
          <div style={{ fontSize: 16, fontWeight: 800, color: C.text }}>⬡ PropAnalyzer</div>
          <div style={{ color: C.muted, fontSize: 10, marginTop: 4, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{user.email}</div>
        </div>
            ...
            <div key={list.id} onClick={() => { setActiveListId(list.id); setView("analyze"); }}
              ...
              {editingListId === list.id ? (
                <input autoFocus value={editingName} onChange={e => setEditingName(e.target.value)} onBlur={() => renameList(list.id)} onKeyDown={e => e.key === "Enter" && renameList(list.id)} onClick={e => e.stopPropagation()}
                  style={{ background: "transparent", border: "none", color: C.text, fontSize: 13, width: "100%", outline: "none" }} />
              ) : (
                <>
          ...
          {showNewList ? (
            <div style={{ marginTop: 8 }}>
              <input autoFocus value={newListName} onChange={e => setNewListName(e.target.value)} onKeyDown={e => e.key === "Enter" && createList()} placeholder="List name..."
                style={{ width: "100%", background: "#070c17", border: `1px solid ${C.accent}`, borderRadius: 7, color: C.text, padding: "7px 10px", fontSize: 12, outline: "none", boxSizing: "border-box" }} />
            ...
            <button onClick={() => setShowNewList(true)} style={{ width: "100%", marginTop: 6, background: "transparent", border: `1px dashed ${C.border}`, borderRadius: 8, color: C.muted, padding: "7px", fontSize: 12, cursor: "pointer" }}>+ New List</button>
          )}
        </div>
        {activeList && (
          <div style={{ padding: "10px", borderTop: `1px solid ${C.border}` }}>
            <div style={{ color: C.muted, fontSize: 10, textTransform: "uppercase", letterSpacing: 1, marginBottom: 7, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{activeList.name}</div>
        ...
        <div style={{ padding: "10px 12px", borderTop: `1px solid ${C.border}` }}>
          <button onClick={signOut} style={{ background: "transparent", border: "none", color: C.muted, fontSize: 12, cursor: "pointer" }}>← Sign Out</button>
        </div>
      </div>
      ...
      <div style={{ flex: 1, overflow: "auto", height: "100vh" }}>
        {!activeList && (
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "100%", textAlign: "center", gap: 16 }}>
            ...
            <div style={{ color: C.muted, maxWidth: 340, lineHeight: 1.7 }}>Create a property list to get started.</div>
        ...
        {activeList && view === "analyze" && (
          <div style={{ padding: "28px 32px" }}>
            <div style={{ marginBottom: 24 }}>
              ...
              <div style={{ color: C.muted, fontSize: 13 }}>AI-powered rental & investment analysis</div>
            </div>
            <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, padding: 24, marginBottom: 28 }}>
              <div style={{ marginBottom: 14 }}><Field label="Property Address" name="address" value={form.address} {...inp} placeholder="123 Ocean Dr, Miami Beach, FL 33139" /></div>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12, marginBottom: 14 }}>
                <Field label="Purchase Price ($)" name="price" value={form.price} {...inp} placeholder="650000" type="number" />
                ...
                <Field label="Sqft" name="sqft" value={form.sqft} {...inp} placeholder="1800" type="number" />
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12, marginBottom: 14 }}>
                <Field label="Monthly HOA ($)" name="hoa" value={form.hoa} {...inp} placeholder="0" type="number" />
                <Field label="Annual Taxes ($)" name="taxes" value={form.taxes} {...inp} placeholder="8500" type="number" />
                <Field label="Annual Insurance ($)" name="insurance" value={form.insurance} {...inp} placeholder="3200" type="number" />
              </div>
              <div style={{ marginBottom: 18 }}>
                <div style={{ color: C.muted, fontSize: 10, textTransform: "uppercase", letterSpacing: 1.2, marginBottom: 5 }}>Notes</div>
                <textarea name="notes" value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} placeholder="Pool, waterfront, recently renovated..."
                  style={{ width: "100%", background: "#070c17", border: `1px solid ${C.border}`, borderRadius: 8, color: C.text, padding: "10px 13px", fontSize: 13, outline: "none", boxSizing: "border-box", height: 68, resize: "vertical", fontFamily: "inherit" }} />
              </div>
              {error && <div style={{ color: C.danger, fontSize: 12, marginBottom: 12 }}>{error}</div>}
              <Btn onClick={analyzeProperty} disabled={analyzing}>{analyzing ? "⏳ Analyzing..." : "⚡ Analyze & Save to List"}</Btn>
            </div>
              ...
              <>
                <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, padding: 20, marginBottom: 20 }}>
                  ...
                  <input value={reportTitle} onChange={e => setReportTitle(e.target.value)}
                    style={{ width: "100%", background: "#070c17", border: `1px solid ${C.border}`, borderRadius: 8, color: C.text, padding: "10px 13px", fontSize: 14, outline: "none", boxSizing: "border-box", fontWeight: 600 }} />
                </div>
                ...
                <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                  ...
                  <div style={{ color: C.muted, fontSize: 12 }}>Opens print dialog — save as PDF</div>
                </div>
              </>
            )}
          </div>
